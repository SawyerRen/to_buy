# To buy 文档


## 商品

### 商品类别列表

Url: http://127.0.0.1:8000/categories/


Method: GET

Params:

```

```

Return:

```json
[
    {
        "id": 1,
        "is_deleted": false,
        "created_at": "2022-03-02T15:50:50Z",
        "updated_at": "2022-03-02T15:51:05Z",
        "name": "Cookies, Cakes, Desserts",
        "description": null
    },
    {
        "id": 2,
        "is_deleted": false,
        "created_at": "2022-03-02T15:50:50Z",
        "updated_at": "2022-03-02T15:51:05Z",
        "name": "Beverage, Milk Tea, Drink Mix\n",
        "description": null
    },
    {
        "id": 3,
        "is_deleted": false,
        "created_at": "2022-03-02T15:50:50Z",
        "updated_at": "2022-03-02T15:51:05Z",
        "name": "Jerky, Dried Seafood, Nuts",
        "description": ""
    },
    {
        "id": 4,
        "is_deleted": false,
        "created_at": "2022-03-02T15:50:50Z",
        "updated_at": "2022-03-02T15:51:05Z",
        "name": "Candy, Jelly, Chocolate",
        "description": null
    },
    {
        "id": 5,
        "is_deleted": false,
        "created_at": "2022-03-02T15:50:50Z",
        "updated_at": "2022-03-02T15:51:05Z",
        "name": "Chips & Crisps",
        "description": null
    }
]
```

### 商品类别列表


Url: http://127.0.0.1:8000/goods/?category_id=0&page=2&page_size=4


Method: GET

Params:

```python
category_id     int # 传0获取所有商品，否则按id查询
page			int # 页数
page_size		int # 页面大小
```

Return:

```json
[
    {
        "id": 5,
        "is_deleted": false,
        "created_at": "2022-03-02T16:36:16Z",
        "updated_at": "2022-03-02T16:36:16Z",
        "name": "Watermelon Stuffed Toast 480g",
        "description": "Watermelon Stuffed Toast 480g",
        "price": "13.99",
        "brand": "SNACK LAB",
        "inventory": 686,
        "sales": 417,
        "image_url": "https://cdn.yamibuy.net/item/e48a8c0c3d06624a296ba46b340759c6_64x64.webp",
        "discount": "1",
        "category": 1
    },
    {
        "id": 6,
        "is_deleted": false,
        "created_at": "2022-03-02T16:36:16Z",
        "updated_at": "2022-03-02T16:36:16Z",
        "name": "Chinese Classic Milk Bread, 480g",
        "description": "Chinese Classic Milk Bread, 480g",
        "price": "6.49",
        "brand": "ASIA FOODS",
        "inventory": 190,
        "sales": 727,
        "image_url": "https://cdn.yamibuy.net/item/73aaa07ee9ce88e58a568c856fe7ec95_64x64.webp",
        "discount": "1",
        "category": 1
    },
    {
        "id": 7,
        "is_deleted": false,
        "created_at": "2022-03-02T16:36:16Z",
        "updated_at": "2022-03-02T16:36:16Z",
        "name": "Breadsticks Cheese Flavor 380g",
        "description": "Breadsticks Cheese Flavor 380g",
        "price": "5.69",
        "brand": "LEJINJI",
        "inventory": 84,
        "sales": 676,
        "image_url": "https://cdn.yamibuy.net/item/aab10275f7280df7ea3456bf0cb40486_64x64.webp",
        "discount": "1",
        "category": 1
    },
    {
        "id": 8,
        "is_deleted": false,
        "created_at": "2022-03-02T16:36:16Z",
        "updated_at": "2022-03-02T16:36:16Z",
        "name": "Baked Cake Chocolate Flavor 95g",
        "description": "Baked Cake Chocolate Flavor 95g",
        "price": "2.29",
        "brand": "Ranli",
        "inventory": 806,
        "sales": 975,
        "image_url": "https://cdn.yamibuy.net/item/ee7bb39029b5f50a41eedcbafa4b5721_64x64.webp",
        "discount": "1",
        "category": 1
    }
]
```


## 用户

### 登录

Url: http://127.0.0.1:8000/account/login

Method: POST

Params:

```python
email     		str # 邮箱
phone_number	str # 电话号码
password		str # 密码
```

Return:

```json
{
    "id": 2,
    "is_deleted": false,
    "created_at": "2022-03-27T22:22:04Z",
    "updated_at": "2022-03-28T09:55:47Z",
    "name": "rsy",
    "password": "123456",
    "phone_number": "111",
    "email": "test2@gmail.com",
    "gender": 1,
    "membership_expire_time": null,
    "membership": 1
}
```

### 注册前检查有没有重复的电话号码或邮箱

Url: http://127.0.0.1:8000/account/register_check?email=test1@gmail.com&phone_number

Method: GET

Params:

```python
email     		str # 邮箱
phone_number	str # 电话号码
```

Return:

```json
{
    "email": "test1@gmail.com",
    "phone_number": "",
    "count": 1
}
```

### 注册

Url: http://127.0.0.1:8000/account/register

Method: POST

Params:

```python
name			str # 用户名
email     		str # 邮箱
phone_number	str # 电话号码
password		str # 密码
password2     	str # 确认密码
gender			int # 性别：1男，2女，0默认


## 购物车  
### 查看某用户购物车
URL：http://127.0.0.1:8000/store/cartitems/?user_id=1  
Method: GET
Params: 
```Json
user_id    int 用户名称
```
Return:  
```Json
{
        "id": 1,
        "user": 1,
        "goods": {
            "id": 1,
            "name": "Majime Millet Biscuits 130g",
            "description": "Majime Millet Biscuits 130g",
            "price": "3.29",
            "brand": "NOMURA",
            "inventory": 3290,
            "sales": 1287,
            "image_url": "https://cdn.yamibuy.net/item/5b7e5659e16cf59f0046e247babe0e3b_64x64.webp",
            "discount": "1",
            "category": 1
        },
        "quantity": 2,
        "total_price": 6.58
    },
    {
        "id": 2,
        "user": 1,
        "goods": {
            "id": 2,
            "name": "Handmade Tiger Skin Cake 110g",
            "description": "Handmade Tiger Skin Cake 110g",
            "price": "3.59",
            "brand": "Ranli",
            "inventory": 4538,
            "sales": 166,
            "image_url": "https://cdn.yamibuy.net/item/542b6e1d182d6703bbd6c89fab1db635_64x64.webp",
            "discount": "1",
            "category": 1
        },
        "quantity": 50,
        "total_price": 179.5
    }
```

### 添加购物车物品(已经有的话为添加，没有为创建)
URL：http://127.0.0.1:8000/store/cartitems/  
Method: POST  
Params:  
```Json
user   int 用户id
goods  int 商品id  
quantity int 商品个数
```
Return:
```Json
{ 
    "id": 1, 
    "user": 1,
    "goods": 1,
    "quantity": 2
}

```

Return:

```json
{
    "id": 14,
    "is_deleted": false,
    "created_at": "2022-03-28T15:41:06.956240Z",
    "updated_at": "2022-03-28T15:41:06.956270Z",
    "name": "test1",
    "password": "123456",
    "phone_number": "123456789",
    "email": "test3@gmail.com",
    "gender": 1,
    "membership_expire_time": null,
    "membership": 1
}
```

## 地址 
### 查看user地址：  
URL： http://127.0.0.1:8000/customers/1/address/  
method=Get
Params:  
```Json
userid int 用户id
```
Return:  
```Json
"count": 2,
    "next": null,
    "previous": null,
    "results": [
        {
            "id": 1,
            "receiver_first_name": "Tristam",
            "receiver_last_name": "Cacacie",
            "state": "Washington",
            "county": "Walla Walla County",
            "street": "20 Stone Corner Plaza",
            "post_code": "57013",
            "receiver_phone_number": "2848080088",
            "is_default": 0,
            "user": 1
        },
        {
            "id": 1001,
            "receiver_first_name": "Tristam1",
            "receiver_last_name": "Cacacie1",
            "state": "Illinois",
            "county": "Walla Walla County1",
            "street": "20 Stone Corner Plaza1",
            "post_code": "57013",
            "receiver_phone_number": "2848080089",
            "is_default": 0,
            "user": 1
        }
    ]
```

### create a new address: 
URL: http://127.0.0.1:8000/customers/1/address/  
Method: POST  
Params: 
```Json
"receiver_first_name": "",
    "receiver_last_name": "",
    "state": "",
    "county": "",
    "street": "",
    "post_code": "",
    "receiver_phone_number": "",
    "user":  #用户id
```
REturn：
```Json
"id": 1002,
    "receiver_first_name": "Tristam1",
    "receiver_last_name": "Cacacie1",
    "state": "Illinois",
    "county": "Walla Walla County1",
    "street": "20 Stone Corner Plaza1",
    "post_code": "57013",
    "receiver_phone_number": "2848080089",
    "is_default": 0,
    "user": 1
```

### modify address:  
URL: http://127.0.0.1:8000/customers/1/address/1  
MEthod: Patch  
Params:  
```Json
user int 用户id
address_id int 地址id
```
REturn：  
```Json
{
    "id": 1,
    "receiver_first_name": "Tristamr",
    "receiver_last_name": "Cacacie",
    "state": "Washington",
    "county": "Walla Walla County",
    "street": "20 Stone Corner Plaza",
    "post_code": "57013",
    "receiver_phone_number": "2848080088",
    "is_default": 0,
    "user": 1
}
```
